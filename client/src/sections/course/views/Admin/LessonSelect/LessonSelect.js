...
const MainFrame = props => {
  ...
  const courseCards = () => {
    ...
    const cards = props.lessons.map(function(data) { 
      ...
      return <CourseCard
        link={"/admin/lesson/update/" + data._id}
        key={data._id}
        courseId={data._id}
        picture={data.videoThumbnail}
...
class AdminLessonSelect extends Component { 
 ...
 state = {
  ...
  async setLessonState(lessonNumber) {
    ...
    let lessons = await Axios.get('/api/courses/lessons?courseId=' + courseId);
    ...
    return (
      ...
      <RequireLogin user={this.props.user} loggedIn={this.props.loggedIn}>
        ...
        <DashboardWrapper user={this.props.user}>
            <MainFrame title="Select Lesson" user={this.props.user} lessons={this.state.lessons} loaded={this.state.loaded} />
        </DashboardWrapper>
